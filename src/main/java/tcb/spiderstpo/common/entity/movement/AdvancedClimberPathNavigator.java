	...
	protected void pathFollow() {
		...
		for(int i = 4; i >= 0; i--) {
			if(this.currentPath.getCurrentPathIndex() + i < this.currentPath.getCurrentPathLength()) {
				...
				if(isOnSameSideAsTarget && (isWaypointInReach || this.isNextTargetInLine(pos, sizeX, sizeY, sizeZ, 1 + i))) {
	...
	private boolean isNextTargetInLine(Vec3d pos, int sizeX, int sizeY, int sizeZ, int offset) {
		if(this.currentPath.getCurrentPathIndex() + offset >= this.currentPath.getCurrentPathLength()) {
			...
			if(!func_237488_a_(pos, currentTarget, 2.0D)) {
				...
				PathPoint pathPoint = this.currentPath.getPathPointFromIndex(this.currentPath.getCurrentPathIndex() + offset);
				Vec3d nextTarget = new Vec3d(pathPoint.x, pathPoint.y, pathPoint.z);
