...
public class ClimberMoveController<T extends MobEntity & IClimberEntity> extends MovementController {
	...
	public void tick() {
		...
		if(this.action == MovementController.Action.STRAFE) {
			...
			Vec3d forwardVector = orientation.getGlobal(this.mob.rotationYaw, 0);
			Vec3d strafeVector = orientation.getGlobal(this.mob.rotationYaw + 90.0f, 0);
			...
			if(this.side != null && verticalOffset > reach - 0.05f && groundDir != this.side && groundDir.getAxis() != this.side.getAxis()) {
				...
				if(hdsq < 0.707f) {
					...
					if(hdsq < 0.1f) {
						jumpDir = new Vec3d(mainOffsetDir.getXOffset(), mainOffsetDir.getYOffset(), mainOffsetDir.getZOffset());
					}
				}
			}
			...
			Vec3d up = orientation.getGlobal(this.mob.rotationYaw, -90);
			...
			if(targetDist < 0.0001D) {
				...
				if(jumpDir == null && this.side != null && targetDist < 0.1D && groundDir == this.side.getOpposite()) {
					jumpDir = new Vec3d(this.side.getXOffset(), this.side.getYOffset(), this.side.getZOffset());
				}
